...
from fabric.api import execute, local, parallel, put, run, sudo, task, env
...
def do_pack():
    ...
    return None if result.failed else f'{archive_dir}{archive_path}'
...
def deploy():
    """Deploy the file"""
    result = execute(do_pack)
    file = list(result.values())[0]
    execute(do_deploy, file)
